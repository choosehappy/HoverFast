...
class TestHoverFast:
    ...
    def test_forward_up_modes(self, up_mode):
        model = HoverFast(in_channels=3, n_classes=2, depth=3, wf=4, up_mode=up_mode)
        x = torch.randn(1, 3, 128, 128)
        out_main, out_aux = model(x)
        assert isinstance(out_main, torch.Tensor)

    def test_forward_batch_norm(self):
        model = HoverFast(in_channels=3, n_classes=2, depth=3, wf=4, batch_norm=True)
        x = torch.randn(1, 3, 128, 128)
        out_main, _ = model(x)
        assert isinstance(out_main, torch.Tensor)
    ...
    def test_output_channels(self):
        model = HoverFast(in_channels=3, n_classes=4, depth=3, wf=4)
        x = torch.randn(1, 3, 128, 128)
        out_main, out_aux = model(x)
        assert out_main.shape[1] == 4

    def test_eval_mode(self):
        model = HoverFast(in_channels=3, n_classes=2, depth=3, wf=4)
        model.eval()
        x = torch.randn(1, 3, 128, 128)
        with torch.no_grad():
            out_main, _ = model(x)
