...
def watershed_object(rg, dist, submarker, opening, offset, region_coord, slide_data,
                     db_output_fname=None, object_class=None):
    ...
    for val in vals:
        ...
        if np.any(np.abs(np.array([cx, cy]) - slide_data['region_size'] // 2) >
...
def region_feature(output_mask, region_coord, dist, marker, opening, slide_data, db_output_fname):
    ...
    for rg in rgs:
        ...
        output += watershed_object(rg, dist[ymin:ymax, xmin:xmax],
                                   marker[ymin:ymax, xmin:xmax] * rg.image,
                                   opening[ymin:ymax, xmin:xmax], (xmin, ymin),
                                   region_coord, slide_data, db_output_fname)

    return output


def post_processing_batch_task(output_tensor, maps_tensor, coords_tensor, slide_data,
                               features_queue, db_output_fname=None):
    ...
    for output_mask, maps, region_coord in zip(output_batch, maps_batch, coords_batch):
        ...
        features = region_feature(output_mask, region_coord, dist, marker, opening,
                                  slide_data, db_output_fname)
        if features:
